<template>
  <div>
    ...
    <!-- antd 반영-->
    <view-create :isLoadingPostAdd="false" :actionSave="actionCreate" />
  </div>
</template>
...
export default {
  ...
  created () {
    logger.debug('viewCreate : ', viewCreate)
    this.initialize()
  },

  methods: {
    initialize () {
      // logger.debug('initialize 아무것도 안함')
    },
    onClickBackBtn () {
    ...
    onSubmitForm (e) {
      if (this.title && this.content && this.author) {
        ...
        logger.debug('this.formErrors : ', this.formErrors)
      }
    },
    prepareData (values) {
      const fdata = {
    ...
    async requestSaveAPI (fdata) {
      ...
      logger.debug('requestAPI returnCondition: ', returnCondition)
    }
  }
