...
<template>
      ...
      <div class="coupon_area transit has_bottom">
        <form>
          ...
          <uu-radio
            ...
            @click="initCoupon = false"
            @change="initCoupon = true"
          >
            <template #label>
              ...
              <div class="badge_area" v-if="couponInfo.cndtUseYn === 'Y'">
                <template v-for="(category, index) in getProductCategoryCode(couponInfo.cndtKeyVal, true)" :key="index">
                  <uu-badge
                ...
                <template
                  v-for="(category, index) in getProductCategoryCode(couponInfo.cndtKeyVal, false)"
                  :key="index"
                >
              ...
              >
                {{ couponTemp.applyTypeNm || '적용 받을 대상' }}
      ...
      <div class="btn_area has_bg">
        <ul class="expected_amount">
          ...
          <li v-if="couponApplied && couponTemp.cpnSno !== ''">
            <p class="title">{{ couponTemp.dcCtt }}</p>
            <p class="primary">
  ...
  <PaymentCouponApplyPopup ref="applyPopup" :init="initCoupon" @update="updateApply" />
</template>
...
import PaymentCouponClearPopup from '@/views/payment/components/PaymentCouponClearPopup.vue';
import PaymentCouponApplyPopup from '@/views/payment/components/PaymentCouponApplyPopup.vue';
...
const initCoupon = ref(true); // 쿠폰 적용대상을 선택 후 다른 쿠폰을 선택 시 적용대상 리셋을 위한 값
...
watch(radio01, () => {
  if (radio01.value) {
    ...
    couponTemp.value = {
      ...
      applyTypeNm:
        orderInfo.value.selSatsList.find((item) => item.busTckKndCd === existingApplyType.value)?.busTckKndNm || '',
      dcKndCd: selectedCoupon?.dcKndCd ?? '',
      dcRt: Number(selectedCoupon?.dcRt ?? '0'),
      applyAmt: couponTemp.value.applyAmt > 0 ? couponTemp.value.applyAmt : 0
    };
...
const apply = (coupon: PaymentCouponModel | undefined) => {
  ...
  if (getMultiTypeReservation.value) {
    ...
    const couponType = orderInfo.value.selSatsList.find((item) => item.busTckCnt > 0);
    couponTemp.value.applyType = couponType?.busTckKndCd || '';
    couponTemp.value.applyTypeNm = couponType?.busTckKndNm || '';
...
const updateApply = (updateType: string) => {
  const selType = orderInfo.value.selSatsList.find((item) => item.busTckKndCd === updateType);
  couponTemp.value.applyType = updateType;
  couponTemp.value.applyTypeNm = selType?.busTckKndNm || '';
  couponTemp.value.applyAmt = selType?.cpnDcntFee || 0;
};
...
// categoryName 출력
const getProductCategoryCode = (queryStr: string, isCategory: boolean) => {
  const regex = isCategory ? /(?:&prdCtgCd=)([^&]+)/ : /(?:&prdThmCd=)([^&]+)/;
...
const categoryName = (categoryId: string, type: string) => {
  // 카테고리일 경우
  if (type === 'category') {
    const category = getCategoryList.value.find((cat) => cat.categoryId === categoryId);
  ...
  if (type === 'theme') {
    for (const category of getCategoryList.value) {
      if (category.subCategories) {
        const theme = category.subCategories.find((theme) => theme.categoryId === categoryId);
        if (theme) {
